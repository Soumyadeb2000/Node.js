function saveExpenses(event) {
    ...
    axios.post("http://65.0.81.231:3000/user/add-expense", obj)
    .then(res => {
        console.log(res);
...
function showOnScreen(data) {
    ...
    deleteBtn.onclick = async () => {
       try {
            await axios.delete(`http://65.0.81.231:3000/user/delete-expense/${data.id}`)
    ...
    //     try {
    //         await axios.put(`http://65.0.81.231:3000/user/update-expense/${data.id}`);
    //         parentElm.removeChild(childElm);
...
window.addEventListener('DOMContentLoaded', async () => {
    try {
        let response = await axios.get('http://65.0.81.231:3000/user/get-expense');
